...
LOOKAHEAD_WPS = 10  # 200 requires too much CPU.
...
class WaypointUpdater(object):
    ...
    def pose_cb(self, pose):
        ...
        if self.waypoints is not None and len(self.waypoints) > 0:
            #rospy.logwarn('[waypoint_updater.py]: len(self.waypoints): %s', len(self.waypoints))
            closest_wp_index = get_closest_index_behind(self.waypoints, pose)
            waypoints_2laps = self.waypoints + self.waypoints
            
            # Get index in self.waypoints of nearest red traffic light:
            ''' TODO: use existing function:
            lights_closest_light_index = get_closest_index_behind(self.lights, pose, False)
            wp_closest_light_index = get_closest_index_behind(self.waypoints, self.lights[lights_closest_light_index].pose, False)
            '''
            # Workaround for now with quick and dirty distance function:
            closest_dist = 1000000.0
            wp_closest_light_index = -1
            for lights_i, waypoint in enumerate(self.lights):
                if self.lights[lights_i].state != 2: # States:0=red, 1=yellow, 2=green, 4=unknown
                    #rospy.logwarn('[waypoint_updater.py]: lights_i:%s /// waypoint: %s', lights_i, self.lights[lights_i])
                    wp_redlight = get_closest_index_behind(self.waypoints, self.lights[lights_i].pose)
                    if wp_redlight > closest_wp_index and self.distance( self.waypoints, closest_wp_index, wp_redlight ) < closest_dist:
                        closest_dist = self.distance( self.waypoints, closest_wp_index, wp_redlight  )
                        wp_closest_light_index = wp_redlight
               # else:
                    #rospy.logwarn('[waypoint_updater.py]: lights_i:%s /// waypoint: %s', lights_i, self.lights[lights_i])                    
            
            # Get distance between car and nearest red traffic light:
            distance_car_redtl = self.distance(self.waypoints, closest_wp_index, wp_closest_light_index)
            
            lane = Lane()
            if 36.0 < distance_car_redtl and distance_car_redtl < 76.0: # red traffic light ahead (26~length of the intersection-->we do not want to stop in the intersection --> 26+10 for braking=36)
                rospy.logwarn('[waypoint_updater.py]: STOP: car:%s, tl:%s, distance:%s', closest_wp_index, wp_closest_light_index, distance_car_redtl)
                lane.waypoints = constant_v_waypoints(waypoints_2laps[closest_wp_index:closest_wp_index+LOOKAHEAD_WPS],-velocity)
            else: # no red traffic light or too late to stop anyway
                rospy.logwarn('[waypoint_updater.py]: DRIVE: car:%s, tl:%s, distance:%s', closest_wp_index, wp_closest_light_index, distance_car_redtl)
                lane.waypoints = constant_v_waypoints(waypoints_2laps[closest_wp_index:closest_wp_index+LOOKAHEAD_WPS],velocity)
                #rospy.logwarn('[waypoint_updater.py]: lane.waypoints:%s', lane.waypoints)
